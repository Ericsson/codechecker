...
from codechecker_report_converter.analyzers.sparse import analyzer_result
    ...
    def test_transform_single_file(self):
        ...
        for test_file in ['sample.c', 'sample.h']:
            ...
            plist_file = os.path.join(
                self.cc_result_dir, f'{test_file}_sparse.plist')

            with open(plist_file, mode='rb') as pfile:
                res = plistlib.load(pfile)

                # Use relative path for this test.
                res['files'][0] = os.path.join('files', test_file)

                self.assertTrue(res['metadata']['generated_by']['version'])
                res['metadata']['generated_by']['version'] = "x.y.z"

            plist_file = os.path.join(
                self.test_files, f'{test_file}.expected.plist')
            with open(plist_file, mode='rb') as pfile:
