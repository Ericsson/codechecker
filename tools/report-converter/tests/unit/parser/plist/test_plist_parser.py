...
class PlistParserTestCase(unittest.TestCase):
    ...
    def test_non_ascii_plist(self):
        """
        Check that plist files containing non-ASCII characters in file paths,
        descriptions, and event messages are parsed without errors.
        """
        non_ascii_plist = os.path.join(
            self.__plist_test_files, 'non_ascii.plist')
        reports = report_file.get_reports(non_ascii_plist)
        self.assertEqual(len(reports), 1)

        report = reports[0]
        self.assertEqual(report.checker_name, 'core.DivideZero')
        self.assertEqual(report.line, 10)
        self.assertEqual(report.column, 5)
        self.assertIn('ünïcödé', report.message)
        self.assertEqual(len(report.bug_path_events), 1)
        self.assertIn('ünïcödé', report.bug_path_events[0].message)
