...
"checker_config": {
  "<analyzer-name>": [{
    "config_name": "<name-of-profile-config-entry>",
...
the `{INSTALL_DIR}/config/config.json` file, out of which the first one with matching
`version_regex` is used when running the analysis with a specific analyzer.

`version_regex` examples:
- match all analyzer versions: "`.*`"
- match analyzer versions 10.0.0 only: "10.0.0"
