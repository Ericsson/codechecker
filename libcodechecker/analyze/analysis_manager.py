...
def init_worker(checked_num, action_num, lock):
    ...
    # TODO: No locking once _old are removed.
    progress_lock = lock


def check_old(check_data):
    """
...
def start_workers_old(args, actions, context, analyzer_config_map, skp_handler,
                      use_db=True):
    """
    ...
    try:
        ...
        pool.map_async(check_old,
                       analyzed_actions,
                       1,
                       callback=lambda results: worker_result_handler(
                           results, {})  # metadata here is not a concept.
                       ).get(float('inf'))
