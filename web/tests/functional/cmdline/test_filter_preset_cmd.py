...
class TestFilterPresetCmdLine(unittest.TestCase):
    ...
    def setup_class(self):
        ...
        # Start or connect to the running CodeChecker server and get
        # connection details.
    ...
    def setup_method(self, _):
        ...
        self.codechecker_cfg = env.import_test_cfg(
            test_workspace)['codechecker_cfg']
        self.server_url = env.parts_to_url(
            self.codechecker_cfg)

        # Get the test project configuration from the prepared
        # test workspace.
        self._testproject_data = env.setup_test_proj_cfg(
            test_workspace)
        self.assertIsNotNone(self._testproject_data)

        # Setup a viewer client to test viewer API calls.
        self._cc_client = env.setup_viewer_client(
            test_workspace)
        ...
        self._test_config = env.import_test_cfg(
            test_workspace)

        # Clean up all filter presets before each test to
        # ensure isolation.
    ...
    def test_filter_preset_cmd_new(self):
        ...
        self.assertEqual(ret, 0)
        self.assertIn(
            "Filter preset 'test_preset' created "
            "successfully.", out)

        # call for list function to check if the preset is
        # listed
        cmd = [self._codechecker_cmd, 'cmd',
               'filter-preset', 'list',
               '--url', str(self.server_url)]
    ...
    def test_filter_preset_cmd_new_no_name(self):
        """
        Test the filter-preset new command line without
        providing a name.
        """
        cmd = [self._codechecker_cmd, 'cmd',
               'filter-preset', 'new', '--name',
               '--url', str(self.server_url)]
        ret, _, err = run_cmd(cmd)

        self.assertEqual(ret, 1)
        self.assertIn(
            "argument --name: expected one argument", err)
    ...
    def test_filter_preset_cmd_new_override(self):
        ...
        cmd = [self._codechecker_cmd, 'cmd',
               'filter-preset', 'new',
               '--name', 'test_preset_override',
               '--url', str(self.server_url)]
        ret, out, _ = run_cmd(cmd)

        self.assertEqual(ret, 0)
        self.assertIn(
            "Filter preset 'test_preset_override' "
        ...
        # create new preset with the same name and check if
        # it fails.
        cmd = [self._codechecker_cmd, 'cmd',
               'filter-preset', 'new',
               '--name', 'test_preset_override',
        ...
        self.assertIn(
            "Filter preset 'test_preset_override' "
            "already exists. Use a different name or "
    ...
    def test_filter_preset_cmd_delete(self):
        ...
        self.assertIn(
            "Filter preset 'test_preset_to_delete' "
            "created successfully.", out1)
    ...
    def test_filter_preset_cmd_delete_non_existing(self):
        """
        Test the filter-preset delete command line with
        non existing id.
        """
    ...
    def test_filter_preset_cmd_list(self):
        ...
        # add 4 presets to the server and 1 that fails due
        # to duplicate name
        cmd1 = [self._codechecker_cmd, 'cmd',
                'filter-preset', 'new',
                '--name', 'test_preset_1',
        ...
        cmd2 = [self._codechecker_cmd, 'cmd',
                ...
                '--name', 'test_preset_2',
                '--url', str(self.server_url)]
        cmd3 = [self._codechecker_cmd, 'cmd',
                'filter-preset', 'new',
                '--name', 'test_preset_3',
                '--url', str(self.server_url)]
        cmd4 = [self._codechecker_cmd, 'cmd',
                'filter-preset', 'new',
                '--name', 'test_preset_4',
                '--url', str(self.server_url)]
        cmd5 = [self._codechecker_cmd, 'cmd',
                'filter-preset', 'new',
                '--name', 'test_preset_4',
                '--url', str(self.server_url)]
        ...
        ret5, out5, _ = run_cmd(cmd5)
        self.assertEqual(ret5, 1)
        self.assertIn(
            "Filter preset 'test_preset_4' already "
            "exists. Use a different name or delete "
            "the existing preset to create a new one.",
            out5)
    ...
    def test_filter_preset_cmd_apply_preset(self):
        ...
        self.assertEqual(ret, 0)
        self.assertIn(
            "Filter preset 'test_preset_to_apply_low' "
            "created successfully.", out)

        # Apply severity presets to the results of the
        # first test project and check if number of reports
        # with the given severity is correct
        cmd1 = [self._codechecker_cmd, 'cmd', 'results',
                self.test_project_name_1,
                '--filter-preset',
                'test_preset_to_apply_low',
                '--url', str(self.server_url)]
        ret1, out1, _ = run_cmd(cmd1)
        ...
        cmd2 = [self._codechecker_cmd,
                'cmd',
                'filter-preset',
                'new',
                '--name',
                'test_preset_to_apply_high',
                '--url', str(self.server_url),
                '--severity', 'high']
        ...
        self.assertEqual(ret2, 0)
        self.assertIn(
            "Filter preset 'test_preset_to_apply_high'"
            " created successfully.", out2)

        # High severity preset.
        cmd3 = [self._codechecker_cmd, 'cmd', 'results',
                self.test_project_name_1,
                '--filter-preset',
                'test_preset_to_apply_high',
                '--url', str(self.server_url)]
    ...
    def test_filter_preset_cmd_apply_preset_no_match(self):
        """
        Test applying a preset that does not match any
        report.
        """

        # Add a preset to the server
        cmd1 = [self._codechecker_cmd,
                'cmd',
                'filter-preset',
                'new',
                '--name',
                'test_preset_to_apply_no_match',
                '--url', str(self.server_url),
                '--severity', 'high']
        ...
        # first test project and check if no reports are
        # listed
        preset_name = \
            'test_preset_to_apply_no_match_DOES_NOT_EXIST'
        cmd = [self._codechecker_cmd,
               'cmd',
               'results',
               self.test_project_name_1,
        ...
        self.assertIn(
            "Filter preset "
            "'test_preset_to_apply_no_match"
            "_DOES_NOT_EXIST' not found!", err)

    def test_filter_preset_cmd_apply_plus_extra_parameters(
            self):
        """
        Test applying a preset with extra parameters that
        contradict the preset parameters.
        """
        ...
        cmd1 = [
            ...
            'new',
            '--name',
            'test_preset_to_apply_plus_extra_parameters',
            '--url', str(self.server_url),
            '--severity', 'low']
        ret1, out1, _ = run_cmd(cmd1)
        ...
        self.assertIn(
            "Filter preset "
            "'test_preset_to_apply_plus_extra"
            "_parameters' created successfully.", out1)
        ...
        self.assertEqual(ret, 1)
        self.assertIn(
            "Cannot combine --filter-preset with "
            "other filter arguments (--severity). "
            "Either use a preset or specify filters "
            "on the command line, not both.", err)
