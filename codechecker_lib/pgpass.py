...
def _match_line(line, hostname, port, database, username):
  '''
  Tries to match the given line to the given hostname, port, database, and username.
...
def get_password_from_file(passfile_path, hostname, port, database, username):
  ...
  For file format see http://www.postgresql.org/docs/current/static/libpq-pgpass.html
  '''

  if len(hostname) == 0 or len(port) == 0 or len(database) == 0  or len(username) == 0:
    return None
