...
class MarkdownlintGenerator(Base):
    """
    Generates documentation URLs for Markdownlint rules based on its Markdown
    documentation file.
    """
    ...
    def generate(self) -> Iterable[Tuple[str, Optional[str]]]:
        ...
        url = "https://github.com/markdownlint/markdownlint/blob/" \
            f"{version}" \
            "/docs/RULES.md"
        for checker, anchor in markdownlint \
                .get_markdownlint_rules(self._http, url):
            yield checker, f"{url}#{anchor}"
