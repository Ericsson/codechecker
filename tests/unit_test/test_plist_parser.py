...
class PlistParserTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.__framework_generated_hashes = \
    ...
    def test_clang38_trunk_plist(self):
        """
        check plist generated by clang 3.8 trunk
        checker name and bug hash should be in the plist file
        """
        ...
        for bug in bugs:
            ...
            if bug.checker_name == 'core.DivideZero':
                self.__core_div_bug_event(bug.get_last_event())
                self.assertEquals(bug.hash_value, '79e31a6ba028f0b7d9779faf4a6cb9cf')
            if bug.checker_name == 'core.StackAddressEscape':
                self.__core_stack_addr_esc_event(bug.get_last_event())
                self.assertEquals(bug.hash_value, 'f7b5072d428e890f2d309217f3ead16f')
            if bug.checker_name == 'deadcode.DeadStores':
                self.assertEquals(bug.hash_value, '8714f42d8328bc78d5d7bff6ced918cc')
