...
bugPerFile = 10
bugLength = 5
fileContent = zlib.compress('\n'.join(['A' * 80] * fileSize), zlib.Z_BEST_COMPRESSION)
#---------------------------------------------------------------------------
...
def parseArgs():
    ...
    parser.add_argument('--output', '-o', dest='output', default=os.path.join(os.path.dirname(__file__), 'results.csv'),
    ...
    parser.add_argument('--check-port', required=True, type=int, dest='check_port',
        help='Check port.')

    parser.add_argument('--view-port', required=True, type=int, dest='view_port',
        help='View port.')
