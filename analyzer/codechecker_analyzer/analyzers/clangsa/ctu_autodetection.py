...
class ClangVersionInfoParser(object):
    ...
    def __init__(self):
        ...
        self.monorepo_source_build_detection_pattern = (
            r'clang version (?P<major_version>[0-9]+)'
            r'\.(?P<minor_version>[0-9]+)\.(?P<patch_version>[0-9]+)\s+'
            r'\((?P<llvm_project_repository>[^\s]*)\s+(?P<llvm_project_commit>[^\s]*)\)\s+'
            r'Target: (?P<target>[^\s]*)\s+'
            r'Thread model: (?P<thread_model>[^\s]*)\s+'
            r'InstalledDir: (?P<installed_dir>[^\s]*).*')
    ...
    def parse(self, version_string):
        ...
        binary version regex is attempted, then the source build.
        """
    ...
    def try_as_monorepo_source_output(self, version_string):
        ...
        clang version 9.0.0 (git@github.com:martong/llvm-project.git
        d021ad9fbeb6d29c8551879f703f45e263e7a700)
        Target: x86_64-unknown-linux-gnu
        Thread model: posix
        InstalledDir: /path/to/clang/bin
        """
