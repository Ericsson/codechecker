...
def handle_failure(
    ...
    # Remove the plist file from a previous successful analysis of this
    # source file so stale results are not stored.
...
def check(check_data):
    ...
    try:
        ...
        def handle_analysis_result(success, zip_file=zip_file):
            """
            Post-analysis actions. On success, stores reports; on failure,
            collects debug files. Reproducer zips go to reproducer_dir;
            failure zips go to failed_dir. --generate-reproducer forces
            reproducer_dir regardless of outcome.
            """
            if success:
            ...
            handle_reproducer(source_analyzer, rh,
                              os.path.join(output_dir_for_zips, zip_file),
                              actions_map)
